...
| `clickup_folder_id` | The ClickUp Delivery-space folder for this client | Routes an entire folder's worth of tasks to this tenant during sync — **this is exactly what broke earlier this session**, when a placeholder "Acme Corp" tenant had the *same* folder id as the real Kenafric tenant, and every synced task landed on the wrong one. That can no longer happen silently: there is now a **partial unique index** on `clickup_folder_id` (`where clickup_folder_id is not null`), so a second tenant claiming the same folder is rejected by Postgres at insert time rather than quietly stealing the first one's tasks. |
...
| `clickup_reports_folder_id` | The client's own "Monthly Progress Reports" folder in Delivery | Added by migration `0029`, after this doc was first written. Reports come from ClickUp **Docs**, and every client's reports folder carries the *identical* name and sits as a **sibling** of the client folder — so neither the name nor `clickup_folder_id` can route a Doc. This column is the mapping `POST /internal/sync/reports` walks. Unique when set, and a check constraint forbids it equalling `clickup_folder_id`. |
